module.exports = do ->
  ...
  propagator_reified_step_bare = (space, left_var_name, right_var_name, bool_name, op_name, inv_op_name) ->
    ...
    if lo is 0 and propagator_step_would_reject inv_op_name, fdvar1, fdvar2
      if hi is 0
        return REJECTED
      fdvar_set_value_inline bool_var, 1
      return SOMETHING_CHANGED
    else if hi is 1 and propagator_step_would_reject op_name, fdvar1, fdvar2
      if lo is 1
        return REJECTED
      ...
      if lo is 1
        return propagator_step_comparison space, op_name, left_var_name, right_var_name
      if hi is 0
        return propagator_step_comparison space, inv_op_name, left_var_name, right_var_name
    ...
    ASSERT (op_reject = propagator_step_would_reject(op_name, fdvar1, fdvar2)) or true
    ASSERT (inv_op_reject = propagator_step_would_reject(inv_op_name, fdvar1, fdvar2)) or true
