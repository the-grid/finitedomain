...
describe "FD - propagators - callback", ->
  ...
  describe 'integration tests', ->
    ...
    it 'should access the vars through the propagator', ->

      cb = (propagator) ->

        rv = domain_get_value propagator.fdvar1.dom
        gv = domain_get_value propagator.fdvar2.dom
        bv = domain_get_value propagator.fdvar3.dom
    ...
    it 'should be able to access the var names array', ->
      ...
      cb = (propagator) ->
        ...
        vars = propagator.from_space.vars

        rv = domain_get_value vars[propagator.target_var_names[0]].dom
        gv = domain_get_value vars[propagator.target_var_names[1]].dom
        bv = domain_get_value vars[propagator.target_var_names[2]].dom
